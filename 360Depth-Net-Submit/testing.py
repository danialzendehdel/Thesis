...
if args.model == '360SDNet':
    ...
    model = LCV_ours_sub3(args.maxdisp, 100)
else:
...
def main():
    ...
    for inx in tqdm(range(len(test_up_img))):
        ...
        if args.real:
            ...
            # ------------------------ ours ---------------------------
            imgU_o = np.array(default_loader(test_up_img[inx], "up"))
            imgD_o = np.array(default_loader(test_down_img[inx], "down"))
            # ---------------------------------------------------------
            # imgD_o = imgD_o[720:1200, 1440:2400, :]
            # imgU_o = imgU_o[720:1200, 1440:2400, :]
