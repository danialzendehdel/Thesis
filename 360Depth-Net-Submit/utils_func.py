...
class Metric(object):
    ...
    def get_info(self):
        return [self.losses.avg, self.EPE.avg, self.RMSELIs.avg, self.RMSELGs.avg, self.ABSRs.avg, self.SQRs.avg,
                self.DELTA.avg,
    ...
    def calculate(self, depth, predict, loss=None):
        ...
        # print(mask.shape)
        # print(depth.shape, predict.shape)
        # mask = (depth >= -0.8858456209395481) * (depth <= 30.0371174029806)
        # print("111:", torch.max(depth[mask]), depth[mask].size())
    ...
    def print(self, iter, token):
        string = '{}:{}\tL {:.3f} EPE {:.3f} RLI {:.3f} RLO {:.3f} ABS {:.3f} SQ {:.3f} DEL {:.3f} DELQ {:.3f} DELC {:.3f}'.format(
...
#     def update(self, info, epoch, tag='Train'):
#         if len(self.key) > 0:
#             self.loss_total.append(epoch, {tag + '/loss_gt': info[0], tag + '/loss_pseudo': info[1],
#                                            tag + '/loss_total': info[2]})
