...
import numpy as np
from models import sub_ASPP
from models.sub_ASPP import convbn, convbn_3d, feature_extraction
...
class LCV(nn.Module):
    def __init__(self, maxdisp, maxdepth):
    ...
    def forward(self, up, down):
        ...
        if self.training:
            ...
            # ------------------------ Upsampliing ------------------------------------------
            cost1 = F.upsample(cost1, [self.maxdepth, 64, up.size()[3]], mode='trilinear')
            cost2 = F.upsample(cost2, [self.maxdepth, 64, up.size()[3]], mode='trilinear')
            # ------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------
        cost3 = F.upsample(cost3, [self.maxdepth, 64, up.size()[3]], mode='trilinear')
        # ------------------------------------------------------------------------------

        cost3 = torch.squeeze(cost3, 1)
        pred3 = F.softmax(cost3, dim=1)
        pred3 = depthregression(self.maxdepth)(pred3)
